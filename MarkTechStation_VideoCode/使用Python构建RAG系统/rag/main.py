...
def save_embeddings(chunks: List[str], embeddings: List[List[float]]) -> None:
    for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
        chromadb_collection.add(
            ...
            ids=[str(i)]  # use chunk index as unique ID
        )
...
# ========== 4. Retrieval ==========
# Given a query, embed it and retrieve the most similar chunks from ChromaDB
...
def retrieve(query: str, top_k: int) -> List[str]:
    ...
    # results['documents'] is a list of lists; [0] gets the first (and only) query's results
...
# Load API key from .env file (replace google.colab.userdata for local use)
load_dotenv()
